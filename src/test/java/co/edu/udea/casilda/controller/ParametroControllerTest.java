...
class ParametroControllerTest {
    ...
    @Test
    void parametroEndpoints() throws Exception {
        ParametroSistemaRepository repository = mock(ParametroSistemaRepository.class);
        when(repository.findByClave(any())).thenReturn(Optional.empty());
        MockMvc m = mvc(new ParametroController(repository));
        m.perform(get("/parametros/max-llamadas-contacto")).andExpect(status().isOk());
        m.perform(put("/parametros/max-llamadas-contacto").param("valor", "3")).andExpect(status().isOk());
